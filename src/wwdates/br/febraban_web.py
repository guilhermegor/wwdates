...
class DatesBRFebrabanWeb(ABCCalendarOperations):
	...
	def __init__(
		...
		"""Initialize the DatesBRFebrabanWeb class.
	...
	# The cached frame covers one specific year range, so the range has to be part of the key —
	# otherwise an instance asking for other years is served the first range that was cached.
	@CacheManager.cache_df(
		key=lambda self: f"br_febraban_holidays_raw_{self.int_year_start}_{self.int_year_end}"
	)
